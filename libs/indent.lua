...
-- LqtIndentationLib.enable(editbox [, colorTable [, tabWidth] ])
...
if not LqtIndentationLib.revision or revision > LqtIndentationLib.revision then
    local lib = LqtIndentationLib
    ...
    local tokenSplitIndent = {}
    lib.tokenSplitIndent = tokenSplitIndent
    tokenSplitIndent[tokens.TOKEN_COLON] = tokens.TOKEN_LINEBREAK
    tokenSplitIndent[tokens.TOKEN_PERIOD] = tokens.TOKEN_LINEBREAK
    ...
    function lib.indentCode(code, tabWidth, colorTable, caretPosition)
        ...
        while true do
            ...
            elseif tokenType == tokens.TOKEN_COLORCODE_START or tokenType == tokens.TOKEN_COLORCODE_STOP or tokenType == tokens.TOKEN_UNKNOWN then
                ...
                local splitIndent = lib.tokenSplitIndent[tokenType]
                if splitIndent then
                    print(previousTokenType)
                end
